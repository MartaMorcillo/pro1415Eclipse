...
public class ListaReservas {
	...
	public void reservar(String nif, String nombre, String tel, int libro,
			int ejemplares) throws ListaLlenaException,
	...
	public  void cancelar2(String nif, int libro) throws ElementoNoEncontradoException{
		boolean enc = false;
		Reserva buscada = new Reserva(nif,null,null,libro);
		for (int i = 0; i < numReservas && !enc; i++) {
			if(lista[i].equals(buscada)){
				enc = true;
				//Elimino la reserva de la posicion i
				for(int j = i; j<numReservas-1 ; j++){
					lista[j] = lista[j+1];
	...
	public void reservasLibro(int codigo){
		for (int i = 0; i < numReservas; i++) {
			if(lista[i].getCodigo() == codigo) {
				...
				System.out.println("Telefono: " + lista[i].getTelefono());
				System.out.println("----");
			}
		}
	}
